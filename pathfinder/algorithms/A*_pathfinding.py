...
def heuristique(node, goal):
    # Dans cet exemple, on peut utiliser la distance entre les noms des nœuds comme heuristique
    # Vous pouvez adapter cette fonction en fonction de votre problème spécifique
    return 1  # Nous utilisons simplement 1 comme heuristique arbitraire

# Fonction pour trouver le chemin le plus court entre deux points en utilisant l'algorithme A*
def trouver_chemin_plus_court(graphe, depart, arrivee):
    try:
        chemin = nx.astar_path(graphe, source=depart, target=arrivee, heuristic=heuristique, weight='weight')
        return chemin
